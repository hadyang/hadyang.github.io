...
**分布式一致性** 主要是针对分布式环境中，针对各节点之间的延迟和节点失效等问题来协调副本之间的状态。可以说分布式一致性与数据复制紧密相关，如果不存在数据的副本，那么数据就是原子一致性的，不存在分布式一致性问题。根据视角的不同，可以将一致性分为数据视角的 **数据一致性模型**，以及以客户端请求为视角的 **客户端一致性模型**。
...
数据一致性模型是为了在多节点并更新的环境中提供一致性的数据视图，根据数据在各节点之间的分布状态，常见的分布式一致性有：线性一致性、因果一致性、顺序一致性以及最终一致性。
...
顺序一致性是相比于线性一致性弱一些的一致性模型，顺序一致性的定义如下：如果对集群的读写操作与每个节点执行这些操作的顺序一致，并且所有操作都是原子的，则认为集群拥有 **顺序一致性**。
...
在客户端环境中，大部分请求都是读取，并且客户端对不一致有较大的容忍度。在大部分的客户端场景中，我们只需要实现最终一致性，以及对单个客户端的一致性。以下讨论的解决方案均基于主从复制，多主复制的情况更为复，但关于一致性模型的定义都是一样的。
